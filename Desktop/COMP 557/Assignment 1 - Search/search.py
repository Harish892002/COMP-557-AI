...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    visited = {}  # Use a dictionary to store the best cost for each state
    ...
    while not priority_queue.isEmpty():
        ...
        if current_state in visited and visited[current_state] <= current_cost:
            continue  # Skip if we've already found a better or equal path to this state

        visited[current_state] = current_cost  # Update the best cost for this state

        for next_state, action, step_cost in problem.getSuccessors(current_state):
            if next_state not in visited or visited[next_state] > current_cost + step_cost:
