...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Step 1: Identify the most likely position for each living ghost
        likelyGhostPositions = [
            max(ghostDist.items(), key=lambda x: x[1])[0]
            for ghostDist in livingGhostPositionDistributions
        ]

        # Step 2: Find the closest ghost based on maze distance
        closestGhost = min(
            likelyGhostPositions,
        ...
        bestAction = min(
            ...
            key=lambda action: self.distancer.getDistance(
                Actions.getSuccessor(pacmanPosition, action), closestGhost
            ),
        )
