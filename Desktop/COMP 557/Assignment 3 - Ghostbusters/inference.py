...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition,
        ...
        """
        # If the ghost is in jail, the only valid observation is None.
        if ghostPosition == jailPosition:
            return 1.0 if noisyDistance is None else 0.0
        else:
            # If the ghost is not in jail, then a None observation is impossible.
            if noisyDistance is None:
                return 0.0
            # Compute the true Manhattan distance between Pacman and the ghost.
            trueDistance = manhattanDistance(pacmanPosition, ghostPosition)
            # Return the probability of the noisy distance given the true distance.
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            # Compute the probability of the observation given the ghost's position
            obsProb = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            # Update belief using Bayes' rule
            newBeliefs[ghostPosition] = obsProb * self.beliefs[ghostPosition]
    ...
    def elapseTime(self, gameState):
        ...
        # Iterate over all previous positions where the ghost could have been
        for oldPos in self.allPositions:
            # Get the probability distribution over new positions
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            # Update the beliefs: sum over all possible new positions
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Distribute particles as evenly as possible
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # Special case: If all weights are zero, reinitialize
        if weights.total() == 0:
            self.initializeUniformly(gameState)
            return
    ...
    def getBeliefDistribution(self):
        ...
        # Count occurrences of each particle position
        for particle in self.particles:
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        # Assign particles by cycling through possible positions
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            weight = 1.0
            # Multiply the probabilities for each ghost's observation
            for i in range(self.numGhosts):
                jailPos = self.getJailPosition(i)
                obsProb = self.getObservationProb(observation[i], pacmanPos, particle[i], jailPos)
                weight *= obsProb
            weights[particle] += weight

        # If all particles have zero weight, reinitialize uniformly.
        if weights.total() == 0:
            self.initializeUniformly(gameState)
            return
