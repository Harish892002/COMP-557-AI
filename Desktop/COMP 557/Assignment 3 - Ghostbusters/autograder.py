...
def evaluate(generateSolutions,
    ...
    for q in test_subdirs:
        ...
        tests = list(
            filter(lambda t: re.match('[^#~.].*\.test\Z', t),
                   os.listdir(subdir_path)))
        tests = list(map(lambda t: re.match('(.*)\.test\Z', t).group(1),
                         tests))
    ...
    return grades.points, grades.maxes
...
def generatePublicTests(moduleDict,
                        privateRoot='private_test_cases',
                        publicRoot='test_cases'):
    import testParser
    import testClasses
    for module in moduleDict:
        setattr(sys.modules[__name__], module, moduleDict[module])
    ...
    for q in sorted(os.listdir(privateRoot)):
        ...
        # create a question object
        questionDict = testParser.TestParser(
            os.path.join(public_subdir_path, 'CONFIG')).parse()
        questionClass = getattr(testClasses, questionDict['class'])
        ...
        for t in sorted(tests):
            ...
            test_out_file = os.path.join(public_subdir_path,
                                         '%s.test_output' % t)
            print("Creating public test case %s from %s" %
                  (public_test_file, test_file))
...
if __name__ == '__main__':
    ...
    if options.runTest != None:
        ...
        points, maxes = evaluate(options.generateSolutions,
                                 options.testRoot,
                                 moduleDict,
                                 ...
                                 display=getDisplay(
                                     options.gradeQuestion != None, options))

        import json
        dict_output = {"tests": []}
        for question in points:
            score = points[question]
            dict_output["tests"].append({
                "name":
                question,
                "max_score":
                maxes[question] * grading.GRADING_SCALAR,
                "score":
                points[question] * grading.GRADING_SCALAR
            })
        try:
