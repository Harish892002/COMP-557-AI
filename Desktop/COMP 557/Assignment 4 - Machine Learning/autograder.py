...
""".format(prereq, q, q, prereq))
    ...
    def end_test(self, pts):
        ...
        if self.points[
                self.current_question] == self.points_at_test_start + pts:
    ...
    def end_q(self):
        ...
        print('\n### Question {}: {}/{} ###'.format(
            ...
            self.points[self.current_question] * GRADING_SCALAR,
            self.maxes[self.current_question] * GRADING_SCALAR))
    ...
    def finalize(self):
        ...
        for q in self.questions:
            print('Question %s: %d/%d' % (q, self.points[q] * GRADING_SCALAR,
                                          self.maxes[q] * GRADING_SCALAR))
        print('------------------')
        print('Total: %d/%d' %
              (sum(self.points.values()) * GRADING_SCALAR,
               sum([self.maxes[q] for q in self.questions]) * GRADING_SCALAR))
...
def parse_options(argv):
    ...
    parser.add_option(
        ...
        action='store_true',
        help=
        'Do not display graphics (visualizing your implementation is highly recommended for debugging).'
    )
...
def main():
    ...
    print('Started at %d:%02d:%02d' % time.localtime()[3:6])
    ...
    for q in questions:
        ...
        for testq, points, fn in TESTS:
            ...
            print(f"\nExecution time: {time_elapsed:.4f} seconds")
        tracker.end_q()
    tracker.finalize()
...
def verify_node(node, expected_type, expected_shape, method_name):
    if expected_type == 'parameter':
        assert node is not None, (
            "{} should return an instance of nn.Parameter, not None".format(
                method_name))
        assert isinstance(node, nn.Parameter), (
            "{} should return an instance of nn.Parameter, instead got type {!r}"
            .format(method_name,
                    type(node).__name__))
    elif expected_type == 'loss':
        assert node is not None, (
            "{} should return an instance a loss node, not None".format(
                method_name))
    ...
    for dimensions in range(1, 10):
        ...
        for point in (random_point, np.zeros_like(random_point)):
            ...
            expected_prediction = np.where(
                np.dot(point,
                       p.get_weights().data.T) >= 0, 1, -1).item()
...
def check_regression(tracker):
    ...
    assert np.isclose(
        train_loss,
        sanity_loss), ("RegressionModel.get_loss() returned a loss of {:.4f}, "
...
def check_lang_id(tracker):
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        inp_xs, inp_y = dataset._encode(dataset.dev_x[start:end],
                                        dataset.dev_y[start:end])
        inp_xs = inp_xs[:word_length]

        output_node = model.run(inp_xs)
        verify_node(output_node, 'node',
