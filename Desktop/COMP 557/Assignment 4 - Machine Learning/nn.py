...
class Parameter(DataNode):
    ...
    def update(self, direction, multiplier):
        ...
        assert direction.data.shape == self.data.shape, (
            ...
            "{}".format(format_shape(direction.data.shape),
...
class AddBias(FunctionNode):
    ...
    def _forward(*inputs):
        ...
        assert inputs[0].shape[1] == inputs[1].shape[1], (
            ...
            "and {}".format(format_shape(inputs[0].shape),
...
class SoftmaxLoss(FunctionNode):
    ...
    def _forward(*inputs):
        ...
        assert np.allclose(np.sum(inputs[1], axis=1),
                           1), ("Labels input must sum to 1 along each row")
...
def as_scalar(node):
    ...
    assert isinstance(
        node,
        Node), ("Input must be a node object, instead has type {!r}".format(
            type(node).__name__))
