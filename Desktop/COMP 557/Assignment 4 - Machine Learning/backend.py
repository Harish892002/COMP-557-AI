    ...
    def iterate_once(self, batch_size):
        for x, y in super().iterate_once(batch_size):
            ...
            if use_graphics and time.time() - self.last_update > 0.1:
                ...
                loss = self.model.get_loss(nn.Constant(self.x),
                                           nn.Constant(self.y)).data
                self.learned.set_data(self.x[self.argsort_x],
                                      predicted[self.argsort_x])
    ...
    def __init__(self, model):
        ...
        if use_graphics:
            ...
            for i in reversed(range(10)):
                ax[i] = plt.subplot2grid((30, 1), (3 * i, 0),
                                         2,
                                         1,
                                         sharex=ax.get(9))
                ...
                for j in range(samples):
                    images[i].append(ax[i].imshow(np.zeros((28, 28)),
                                                  vmin=0,
                                                  vmax=1,
                                                  cmap="Greens",
                                                  alpha=0.3))
                    texts[i].append(ax[i].text(0,
                                               0,
                                               "",
                                               ha="center",
                                               va="top",
...
class LanguageIDDataset(Dataset):
    ...
    def __init__(self, model):
        ...
        self.epoch = 0
        self.bucket_weights = self.train_buckets[:, 1] - self.train_buckets[:,
                                                                            0]
        self.bucket_weights = self.bucket_weights / float(
            self.bucket_weights.sum())
...
alphabet above have been substituted with ASCII symbols.""".strip())
        ...
        self.predicted_template = u"Pred: {:<NUM}".replace(
            'NUM', str(max_lang_len))

        self.word_template = u"  "
        self.word_template += u"{:<NUM} ".replace('NUM', str(max_word_len))
        self.word_template += u"{:<NUM} ({:6.1%})".replace(
            'NUM', str(max_lang_len))
        self.word_template += u" {:<NUM} ".replace(
    ...
    def iterate_once(self, batch_size):
        ...
        for iteration in range(self.train_x.shape[0] // batch_size):
            bucket_id = np.random.choice(self.bucket_weights.shape[0],
                                         p=self.bucket_weights)
            example_ids = self.train_buckets[bucket_id, 0] + np.random.choice(
                self.train_buckets[bucket_id, 1] -
                self.train_buckets[bucket_id, 0],
                size=batch_size)

            yield self._encode(self.train_x[example_ids],
                               self.train_y[example_ids])
            ...
            if use_graphics and time.time() - self.last_update > 0.5:
                ...
                print("epoch {:,} iteration {:,} validation-accuracy {:.1%}".
                      format(self.epoch, iteration, dev_accuracy))

                for idx in self.spotlight_idxs:
                    correct = (dev_predicted[idx] == dev_correct[idx])
                    word = u"".join([
                        self.chars_print[ch] for ch in self.dev_x[idx]
                        if ch != -1
                    ])
